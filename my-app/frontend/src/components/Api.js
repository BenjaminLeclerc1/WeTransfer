...
export const getFiles = async () => {
  try {
    const response = await axios.get(`${API_URL}/files/myfiles`); // Utilisation de la route /myfiles
...
export const createShareLink = async (fileId) => {
  try {
    const response = await axios.get(`${API_URL}/files/${fileId}/share`); // Utilisation de la route /:fileId/share
    return response.data; // Retourner les données de la réponse
  } catch (error) {
